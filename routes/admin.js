...
router.get("/menu", checkAdminToken, (req, res) => {
  try {
...
router.get("/locations", checkAdminToken, (req, res) => {
  try {
    const dbPath = path.join(__dirname, "../data/db.json");
...
router.patch("/menu-items/:id/availability", checkAdminToken, (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (available) {
      ...
      // Remove location
      db.menuItems[menuItemIndex].availableAt = db.menuItems[menuItemIndex].availableAt.filter(
        loc => loc !== locationId
      );
    }
    ...
    res.json({ 
      success: true, 
      message: `Product ${available ? 'enabled' : 'disabled'} for location ${locationId}` 
    });
  } catch (error) {
...
router.post("/menu-items/:id/image", checkAdminToken, upload.single('image'), (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (db.menuItems[menuItemIndex].image) {
      const oldImagePath = path.join(__dirname, "../uploads", path.basename(db.menuItems[menuItemIndex].image));
...
router.delete("/menu-items/:id/image", checkAdminToken, (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (db.menuItems[menuItemIndex].image) {
      const imagePath = path.join(__dirname, "../uploads", path.basename(db.menuItems[menuItemIndex].image));
...
router.get("/stats", checkAdminToken, (req, res) => {
  res.json({ 
